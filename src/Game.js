...
function Game() {
  //STATE SECTION
	...
	const [oppSnake, setOppSnake] = useState([44])
	const [oppFood, setOppFood] = useState({type: 'speedUp', index: 0})
...
*/
  ...
  useEffect(() => {
		...
		socket.on('updateOpponentSnake', newSnake => {
			console.log(newSnake);
			setOppSnake(newSnake)
			// console.log("oppSnake: ", oppSnake);
		})
      ...
      if(e.key === 'ArrowUp' || e.key === 'ArrowDown' || e.key === 'ArrowLeft' || e.key === 'ArrowRight')
		...
		return () => {
			socket.off('connect');
			socket.off('updateOpponentSnake');
		}
	}, []);
		...
		<div className="bg-slate-500  w-full min-h-screen flex flex-col justify-center items-center">
			...
			<div className="w-[600px] h-[600px] border border-black flex flex-wrap">
				{boardArray.map((tile, i) => {
					let tt;
					if (oppSnake.includes(i)) {
						tt = "w-[10%] h-[10%] border bg-black";
					} else if (i === oppFood.index) {
						if (oppFood.type === "speedUp") {
							tt = "w-[10%] h-[10%] border bg-green-500";
						} else if (oppFood.type === "shorten") {
